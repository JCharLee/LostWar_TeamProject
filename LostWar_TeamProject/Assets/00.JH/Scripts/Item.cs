    ...
    public class Clothes : Item// 아이템 클래스를 상속받는 방어구 장비
        {
            ...
            public Clothes(string _name, int _str, int _con, int _vit, float _def, float _weight,  ItemType _itemType)//이름, 능력치, 텍스쳐, 부위()
            {
                itemType = _itemType;
                name = _name;str = _str;con = _con;vit = _vit;def = _def;weight = _weight;
            }
        }
    [System.Serializable]
